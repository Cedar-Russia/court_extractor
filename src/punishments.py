...
class PunishmentExtractor:
    ...
    def find_punishment(self, input_string):
           ...
           {{
               "Person Name": {{
                   "1": {{
                       ...
                       "suspended":<value from {self.punishments_data} suspended field>",
                       "severity": {{
                           "years": <number>,
                           "months": <number>,
                           "rubles": <number>,
        ...
        4. Suspended rules:
           - If условно (считать условным) for given punishment than условное
           - If не находит основания для применения положений ст. 73 УК РФ than реальное
           - If there is no mention of условное than реальное

        5. Return None if no punishment information found
